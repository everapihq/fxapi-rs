pub mod baseline {
    ...
    const BASE_URL: &str = "https://api.fxapi.com/v1/";

    pub fn construct_client(
        user_agent: Option<&str>,
        settings: &api::Settings,
    ) -> Result<Client, FxapiError> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|err| FxapiError::ClientConstruction { source: err })?;
        Ok(client)
    }
    ...
    ) -> Result<Url, FxapiError> {
        let mut url = Url::parse(BASE_URL).map_err(|_| FxapiError::UrlConstruction)?;
